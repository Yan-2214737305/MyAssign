...
bool solveMazeRec(Grid<bool>& maze, Stack<GridLocation> &path, Set<GridLocation> &visited) {
    ...
    if (path.peek() == mazeExit) {
        ...
        auto nextLoc = generateValidMoves(maze, path.peek()).difference(visited);
        for (const GridLocation &item : nextLoc) {
            //choose
            path.push(item);
            visited.add(item);
            ...
            //unchoose
            path.pop();
            MazeGraphics::highlightPath(path, "green", 5);
        }
    }
    return false;
}
...
Stack<GridLocation> solveMaze(Grid<bool>& maze) {
    ...
    Set<GridLocation> visited;
    solveMazeRec(maze, path, visited);
    return path;
}
#endif
